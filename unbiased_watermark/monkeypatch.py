...
def patch_model(model: GenerationMixin):
    if getattr(model, "__unbiased_pathced__", False):
        # already patched
        return
    ...
    def _get_logits_warper(self, *args, **kargs):
        ...
        if "logits_warper" in context:
            warpers = self._merge_criteria_processor_list(warpers, context["logits_warper"])
        return warpers
    ...
    setattr(model, "__unbiased_pathced__", True)
