...
class Rephrase:
    def __init__(self, args):
        ...
        self.watermark_detector_original = WatermarkDetector(vocab=list(self.tokenizer.get_vocab().values()),
        ...
        self.watermark_detector_new = WatermarkDetector(vocab=list(self.tokenizer.get_vocab().values()),
                                                    gamma=self.settings['gamma'],  # should match original setting
                                                    seeding_scheme=self.settings['seeding_scheme'],  # should match original setting
                                                    device=self.model.device,  # must match the original rng device type
                                                    tokenizer=self.tokenizer,
    ...
    def rephrase(self,):
        ...
        with jsonlines.open(file_path, mode='w') as writer:
            ...
            for data in tqdm(self.json_list):
                ...
                writer.write({'z_score_ori': round(score_dict_ori['z_score'], 4), 'z_score_new': round(score_dict_new['z_score'], 4), 'original_text': data['generated_text'], 'generated_text': output_text})
